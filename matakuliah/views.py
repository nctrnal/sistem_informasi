...
def hapus_matkul(request, pk):
    ...
    if request.method == 'POST':
        mata_kuliah.delete()
        messages.success(request, f'Mata kuliah "{mata_kuliah.nama_mata_kuliah}" berhasil dihapus.')
...
def tambah_mata_kuliah(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('matakuliah')  # Ganti 'daftar_matkul' dengan nama URL untuk halaman daftar mata kuliah
    else:
        form = MataKuliahForm()
